...
from qnet.algebra import (
    identity_matrix, Circuit, SLH, LocalSpace, Destroy, ZeroOperator,
    IdentityOperator, BasisKet, LocalKet, TensorKet, LocalSigma,
    CoherentStateKet)
from qnet.circuit_components.pseudo_nand_cc import PseudoNAND

from trajectorydata import TrajectoryData
import qnet_qsd
from qnet_qsd import (
    local_ops, find_kets, QSDCodeGen, QSDOperator,
    QSDCodeGenError, expand_cmd, compilation_worker, qsd_run_worker,
    _find_time_dependent_coeffs)
...
def slh_Sec6():
    ...
    H = (
        E*I*(Ac1-A1) + 0.5*chi*I*(Ac1*Ac1*A2 - A1*A1*Ac2) +
...
def Sec6_codegen(slh_Sec6, slh_Sec6_vals):
    ...
    codegen.set_trajectories(
        psi_initial=psi0*psi1*psi2, stepper='AdaptiveStep', dt=0.01,
        nt_plot_step=100, n_plot_steps=5, n_trajectories=1, traj_save=10)
    return codegen
...
def test_qsd_codegen_initial_state(slh_Sec6):
    ...
    psi = ( ((psi_cav1(0) + psi_cav1(1)) / sympy.sqrt(2))
          ...
          * psi_spin(0) )
    codegen.set_trajectories(
        psi_initial=psi, stepper='AdaptiveStep', dt=0.01, nt_plot_step=100,
        n_plot_steps=5, n_trajectories=1, traj_save=10)
...
def test_qsd_codegen_traj(slh_Sec6):
    ...
    codegen.set_trajectories(
        psi_initial=None, stepper='AdaptiveStep', dt=0.01, nt_plot_step=100,
        n_plot_steps=5, n_trajectories=1, traj_save=10)
    scode = codegen._trajectory_lines(indent=0)
    assert dedent(scode).strip() == dedent(r'''
...
def test_expand_cmd(monkeypatch):
    ...
    cmd = ["$CC", "-O2", "-I$PREFIX/include/qsd", "-o qsd-example",
           "$HOME/qsd-example.cc", "-L$PREFIX/lib", "-lqsd"]
    assert " ".join(expand_cmd(cmd)) == (
        '$CC -O2 -I/home/qnet/local/include/qsd -o qsd-example '
        '/home/qnet/qsd-example.cc -L/home/qnet/local/lib -lqsd')
...
def test_compilation_worker(tmpdir, monkeypatch, Sec6_codegen):
    ...
    kwargs = {
        'executable': 'run_qsd1', 'path': '~/tmp', 'cc_code': scode,
        'keep_cc': True, 'cmd': cmd}

    def runner1(cmd, stderr, cwd):
        """simulate invocation of the compiler"""
        assert " ".join(expand_cmd(cmd)) == (
            'CC -O2 -I' + PREFIX + '/include/qsd -o run_qsd1 run_qsd1.cc -L' +
            PREFIX + '/lib -lqsd')
...
def test_qsd_run_worker(datadir, tmpdir, monkeypatch):
    ...
    # run the worker
    kwargs = {
        'executable': 'run_qsd', 'path': '~/bin', 'seed': 232334,
        'operators': {'X1': 'X1.out', 'X2': 'X2.out'},
...
def test_compile(Sec6_codegen):
    ...
    with pytest.raises(ValueError) as exc_info:
        traj.compile(
            qsd_lib='~/local/lib', qsd_headers='~/local/header',
...
@mock.patch(
    'qnet_qsd.compilation_worker', return_value='/home/qnet/bin/qsd_test')
def test_compilation_worker(
        mock_compilation_worker, Sec6_codegen, traj1, traj2_10):
    ...
    codegen.compile(
        ...
        keep_cc=False)
    comp_kwargs = {
        'executable': 'qsd_test', 'path': '~/bin', 'cc_code': str(codegen),
        'keep_cc': False, 'cmd': [
            '$CC', '-g', '-O0', '-I~/local/header', '-o', 'qsd_test',
            'qsd_test.cc', '-L~/local/lib', '-lqsd']}
    ...
    qsd_run_worker = 'qnet_qsd.qsd_run_worker'
    traj1_ID = traj1.ID
